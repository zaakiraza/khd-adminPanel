...
export default function StudentDetailView() {
  ...
  const fetchClassesAndSessions = async () => {
    try {
      // Fetch all classes
      const classesResponse = await axios.get(`${baseURL}/class/all`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      ...
      // Fetch all sessions
      const sessionsResponse = await axios.get(`${baseURL}/session/all`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      if (sessionsResponse.status === 200) {
        ...
        sessions.forEach((session) => {
          sessionMap[session._id] = session.session_name;
        });
        setSessionsMap(sessionMap);
      }
    } catch (e) {
      console.error("Failed to fetch classes or sessions:", e);
    }
  };
          ...
          <div className="detail-section">
            ...
            <div className="class-history-list">
              {student.class_history.map((classItem, index) => {
                const className = classesMap[classItem.class_name] || classItem.class_name || "N/A";
                const sessionName = sessionsMap[classItem.session] || classItem.session || "N/A";
                return (
                  <div key={classItem._id || index} className="class-history-item">
                    <p>
                      <strong>Class:</strong> {className} | 
                      <strong> Year:</strong> {classItem.year || "N/A"} | 
                      <strong> Session:</strong> {sessionName} | 
                      <strong> Status:</strong> {classItem.status || "N/A"} | 
                      <strong> Result:</strong> {classItem.result || "N/A"} | 
                      <strong> Repeat Count:</strong> {classItem.repeat_count || 0} | 
                      <strong> Completed:</strong> {classItem.isCompleted ? "Yes" : "No"}
                    </p>
                  </div>
                );
              })}
